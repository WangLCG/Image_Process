  ...
  int search_range)
{
  ...
  {        /* 1.0 = 0+1 */
    tc->nPyramidLevels = 1;
  } else if (subsampling <= 3.0)  
  {    /* 3.0 = 2+1 */
    tc->nPyramidLevels = 2;
    tc->subsampling = 2;
  } else if (subsampling <= 5.0)  
  {    /* 5.0 = 4+1 */
    tc->nPyramidLevels = 2;
    tc->subsampling = 4;
  } else if (subsampling <= 9.0)  
  {    /* 9.0 = 8+1 */
    tc->nPyramidLevels = 2;
    tc->subsampling = 8;
  } else 
  ...
  KLT_TrackingContext tc)
{
  ...
  /* Compute the num of invalid pixels at each level of the pyramid.
     计算每层金字塔的无效像素数量    
     ...
     第1层有5个无效像素意味着在第0层有5*subsampling个无效像素     
     */
  n_invalid_pixels = smooth_gauss_hw;  //边界像素个数认为是无效像素个数    
  ...
  tc->borderx = border;  //第0层边界X方向像素个数, 边界    
  tc->bordery = border;
}

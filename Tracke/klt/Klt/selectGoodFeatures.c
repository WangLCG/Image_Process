  ...
  KLT_BOOL overwriteAllFeatures)
{
  ...
  uchar *featuremap; /* Boolean array recording proximity of features 二值图,有特征值得地方值为1，否则为0  */   
  int *ptr;
  ...
  if (!overwriteAllFeatures)
    for (indx = 0 ; indx < featurelist->nFeatures ; indx++)
      if (featurelist->feature[indx]->val >= 0)  //上一次计算保留的特征点    
      {
        x   = (int) featurelist->feature[indx]->x;
        y   = (int) featurelist->feature[indx]->y;
        _fillFeaturemap(x, y, featuremap, mindist, ncols, nrows);  //坐标(x,y)，大小mindist范围内填充1    
      }
  ...
  selectionMode mode)
{
  ...
  } else  
  {
    ...
    if (tc->smoothBeforeSelecting)  //高斯平滑滤波
    {
